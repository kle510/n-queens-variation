...
def generate_children(nursery, depth):
    ...
    while tree_count > 0:
        ...
        # Get all child combinations up to tree. Add one to account for all 0's case
        ...
        if starting_pos != 0:
            ...
            # We subtract one from possible combos because we don't want to rewrite the original childrens list
        ...
        for index, item in enumerate(range(starting_pos, curr_tree)):
            ...
            sublist[index] = 1
            # Make a list for for all 0's case
            zerolist = [0] * (open_space)
            # If there are no children, add directly to list
            if starting_pos == 0:
                if index == (curr_tree - 1):
                    ...
                    children.append(sublist)
            # If there are children, add to the back of children
            else:
...
def simulated_annealing(initial_state_lizard_loc, dim, lizard_num):
    ...
    while current_temp > 0:
        ...
        # We want to minimize energy i.e. get a negative number
...
def generate_neighbor(current_state, current_lizard_loc, dim, lizard_num):
    ...
    # Pick a random lizard and set old random lizard position to 0
